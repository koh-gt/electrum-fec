...
GIT_REPO=https://github.com/koh-gt/electrum-fec
...
cp -f "$DLL_TARGET_DIR/libsecp256k1.0.dylib" "$PROJECT_ROOT/electrum_fec/" || fail "Could not copy libsecp256k1 dylib"
...
cp -f "$DLL_TARGET_DIR/libzbar.0.dylib" "$PROJECT_ROOT/electrum_fec/" || fail "Could not copy ZBar dylib"
...
cp -f "$DLL_TARGET_DIR/libusb-1.0.dylib" "$PROJECT_ROOT/electrum_fec/" || fail "Could not copy libusb dylib"
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-fec-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-fec-${VERSION}.dmg"
