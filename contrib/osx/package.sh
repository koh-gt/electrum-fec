...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/electrum-fec-macos
    cd /tmp/electrum-fec-macos
...
rm -rf /tmp/electrum-fec-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-fec-macos/image/
cp -r $1 /tmp/electrum-fec-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-FEC_uncompressed.dmg \
    /tmp/electrum-fec-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-FEC_uncompressed.dmg electrum-fec-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-FEC_uncompressed.dmg

echo "Done."
sha256sum electrum-fec-$VERSION.dmg
