...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE="$CONTRIB_WINE/fresh_clone/electrum-fec" && \
        rm -rf "$FRESH_CLONE" && \
        umask 0022 && \
...
docker run -it \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/wine64/drive_c/electrum-fec \
    --rm \
    --workdir /opt/wine64/drive_c/electrum-fec/contrib/build-wine \
