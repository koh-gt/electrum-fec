...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-fec-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\electrum_fec\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-fec\*.*"
  File "..\..\electrum_fec\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-fec-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-fec-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-fec-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-fec-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-fec-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\ferrite" "" "URL:ferrite Protocol"
  WriteRegStr HKCU "Software\Classes\ferrite" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\ferrite" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\ferrite\shell\open\command" "" "$\"$INSTDIR\electrum-fec-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\ferrite"
