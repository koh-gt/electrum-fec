...
pushd $WINEPREFIX/drive_c/electrum-fec
...
LOCALE="$WINEPREFIX/drive_c/electrum-fec/electrum_fec/locale/"
...
ELECTRUM_CMDLINE_NAME="$NAME_ROOT-$VERSION" wine "$WINE_PYHOME/scripts/pyinstaller.exe" --noconfirm --ascii --clean -w deterministic.spec --name electrum-ferrite
...
cd dist
mv electrum-fec-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
