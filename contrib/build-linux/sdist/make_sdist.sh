...
(
    ...
    rm -rf "$LOCALE"
    cp -r "$CONTRIB/deterministic-build/electrum-fec-locale/locale/" "$LOCALE/"
    if ([ "$OMIT_UNCLEAN_FILES" != 1 ]); then
...
if ([ "$OMIT_UNCLEAN_FILES" = 1 ]); then
    ...
    rm -f "$PROJECT_ROOT/electrum_fec/paymentrequest_pb2.py"
fi
...
os.rename(f"dist/_sourceonly/Electrum-FEC-{VER}.tar.gz", f"dist/Electrum-FEC-sourceonly-{VER}.tar.gz")
EOF
        rmdir "$PY_DISTDIR"
    fi
)
