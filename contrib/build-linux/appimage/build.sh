...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE="$CONTRIB_APPIMAGE/fresh_clone/electrum-fec" && \
        rm -rf "$FRESH_CLONE" && \
        umask 0022 && \
...
docker run -it \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum-fec \
    --rm \
    --workdir /opt/electrum-fec/contrib/build-linux/appimage \
