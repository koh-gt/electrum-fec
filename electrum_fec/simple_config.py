...
class SimpleConfig(Logger):
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'fec':8, 'mfec':5, 'ufec':2, 'bits':2, 'sat':0}
    ...
    def get_wallet_path(self, *, use_gui_last_wallet=False):
        ...
        old_path = os.path.join(self.path, "electrum-fec.dat")
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-fec.conf into user_config[]."""
    if not path:
        return {}
