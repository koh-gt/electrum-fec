...
from electrum_fec.invoices import (PR_DEFAULT_EXPIRATION_WHEN_CREATING,
                                   PR_PAID, PR_UNKNOWN, PR_EXPIRED, PR_INFLIGHT,
                                   pr_expiration_values, Invoice)
from electrum_fec import bitcoin, constants
from electrum_fec import lnutil
from electrum_fec.transaction import tx_from_any, PartialTxOutput
from electrum_fec.util import (parse_URI, InvalidBitcoinURI, TxMinedInfo, maybe_extract_lightning_payment_identifier,
                               InvoiceError, format_time, parse_max_spend, BITCOIN_BIP21_URI_SCHEME)
from electrum_fec.lnaddr import lndecode, LnInvoiceException
from electrum_fec.lnurl import decode_lnurl, request_lnurl, callback_lnurl, LNURLError, LNURL6Data
...
from electrum_fec.gui.kivy import KIVY_GUI_PATH
from electrum_fec.gui.kivy.i18n import _

if TYPE_CHECKING:
    from electrum_fec.gui.kivy.main_window import ElectrumWindow
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Ferrite address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Ferrite Address') + ':\n' + address)
                        return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_fec.util import create_bip21_uri
        amount = self.app.get_amount(self.amount)
