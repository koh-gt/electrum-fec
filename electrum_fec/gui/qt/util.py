...
from electrum_fec.i18n import _, languages
from electrum_fec.util import FileImportFailed, FileExportFailed, make_aiohttp_session, resource_path
from electrum_fec.util import EventListener, event_listener
from electrum_fec.invoices import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING, PR_UNCONFIRMED
from electrum_fec.logging import Logger
from electrum_fec.qrreader import MissingQrDetectionLib
...
def export_meta_gui(electrum_window: 'ElectrumWindow', title, exporter):
    ...
    filename = getSaveFileName(
        ...
        filename='electrum-fec_{}.json'.format(title),
        filter=filter_,
        config=electrum_window.config,
    )
    if not filename:
