...
from electrum_fec.invoices import (PR_UNPAID, PR_EXPIRED, PR_UNKNOWN, PR_PAID, PR_INFLIGHT,
                                   PR_FAILED, PR_ROUTING, PR_UNCONFIRMED)
from electrum_fec.lnaddr import LnInvoiceException
from electrum_fec.logging import get_logger
from electrum_fec.transaction import PartialTxOutput
from electrum_fec.util import (parse_URI, InvalidBitcoinURI, InvoiceError,
                               maybe_extract_lightning_payment_identifier)
from electrum_fec.lnurl import decode_lnurl, request_lnurl, callback_lnurl
...
class QEUserEnteredPayment(QEInvoice):
    ...
    def validate(self):
        ...
        if not bitcoin.is_address(self._recipient):
            self.validationError.emit('recipient', _('Invalid Ferrite address'))
            return
