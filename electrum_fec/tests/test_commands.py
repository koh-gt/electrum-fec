...
from electrum_fec.commands import Commands, eval_bool
from electrum_fec import storage, wallet
from electrum_fec.wallet import restore_wallet_from_text
from electrum_fec.address_synchronizer import TX_HEIGHT_UNCONFIRMED
from electrum_fec.simple_config import SimpleConfig
from electrum_fec.transaction import Transaction, TxOutput, tx_from_any
...
class TestCommands(ElectrumTestCase):
    ...
    def test_export_private_key_imported(self, mock_save_db):
        ...
        with self.assertRaises(Exception):
            cmds._run('getprivatekeys', ("fec1qgfam82qk7uwh5j2xxmcd8cmklpe0zackqwq8jp",), wallet=wallet)  # not in wallet
        self.assertEqual("p2wpkh:TAa25Tq4PdzhDKBoVaFaCdV3yxvLrRikQviNkuFQLeYopsVvNTV3",
                         cmds._run('getprivatekeys', ("fec1q2ccr34wzep58d4239tl3x3734ttle92arvely7",), wallet=wallet))
        # list of addresses tests
        with self.assertRaises(Exception):
            cmds._run('getprivatekeys', (['fec1q2ccr34wzep58d4239tl3x3734ttle92arvely7', 'asd'],), wallet=wallet)
        self.assertEqual(['p2wpkh:TAa25Tq4PdzhDKBoVaFaCdV3yxvLrRikQviNkuFQLeYopsVvNTV3', 'p2wpkh:TAgoypi14k5Y54svysG62xp5QFRWiF1W64zxaFRFPo2jMPSMoa5D'],
                         cmds._run('getprivatekeys', (['fec1q2ccr34wzep58d4239tl3x3734ttle92arvely7', 'fec1q9pzjpjq4nqx5ycnywekcmycqz0wjp2nq7n0qjr'],), wallet=wallet))
    ...
    def test_export_private_key_deterministic(self, mock_save_db):
        ...
        self.assertEqual("p2wpkh:T6v5Q8KEmjLmJoTxPfXfyNcCEFYC7Lfmwmp9Y8dce9knevo9ZkPk",
                         cmds._run('getprivatekeys', ("fec1q3g5tmkmlvxryhh843v4dz026avatc0zz8xd49e",), wallet=wallet))
        # list of addresses tests
        with self.assertRaises(Exception):
            cmds._run('getprivatekeys', (['fec1q3g5tmkmlvxryhh843v4dz026avatc0zz8xd49e', 'asd'],), wallet=wallet)
        self.assertEqual(['p2wpkh:T6v5Q8KEmjLmJoTxPfXfyNcCEFYC7Lfmwmp9Y8dce9knevo9ZkPk', 'p2wpkh:TAgoypi14k5Y54svysG62xp5QFRWiF1W64zxaFRFPo2jMPSMoa5D'],
                         cmds._run('getprivatekeys', (['fec1q3g5tmkmlvxryhh843v4dz026avatc0zz8xd49e', 'fec1q9pzjpjq4nqx5ycnywekcmycqz0wjp2nq7n0qjr'],), wallet=wallet))
...
class TestCommandsTestnet(TestCaseForTestnet):
    ...
    def test_serialize(self):
        ...
        jsontx = {
            ...
            "outputs": [
                {
                    "address": "tfec1q4s8z6g5jqzllkgt8a4har94wl8tg0k9m77w2jy",
                    "value_sats": 990000
                }
            ]
        }
    ...
    def test_payto(self, mock_save_db):
        ...
        tx_str = cmds._run(
            'payto', (),
            destination="tfec1qsyzgpwa0vg2940u5t6l97etuvedr5dejcp84ad",
            amount="0.00123456",
            feerate=50,
            locktime=1972344,
            wallet=wallet)
        ...
        txout = TxOutput.from_address_and_value("tfec1qsyzgpwa0vg2940u5t6l97etuvedr5dejcp84ad", 123456)
    ...
    def test_paytomany_multiple_max_spends(self, mock_save_db):
        ...
        tx_str = cmds._run(
            'paytomany', (),
            outputs=[["tfec1qk3g0t9pw5wctkzz7gh6k3ljfuukn729srkx29s", 0.002],
                     ["tfec1qr7evucrllljtryam6y2k3ntmlptq208p3lzp67", "2!"],
                     ["tfec1qs3msqp0n0qade2haanjw2dkaa5lm77vw4sm3l7", 0.003],
                     ["tfec1qar4ye43tdfj6y5n3yndp9adhs2wuz2v0hqzdyk", "3!"]],
            fee="0.00005000",
            locktime=2094054,
            wallet=wallet)
