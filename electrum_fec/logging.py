...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_fec."):
...
def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_fec_log_*.log")), reverse=True)
    for f in files[keep:]:
        try:
            os.remove(str(f))
        except OSError as e:
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_fec_log_{timestamp}_{PID}.log"
...
electrum_logger = logging.getLogger("electrum_fec")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_fec."):
        name = name[13:]
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    _logger.info(f"Electrum-FEC version: {ELECTRUM_VERSION} - https://electrum-fec.org - {GIT_REPO_URL}")
